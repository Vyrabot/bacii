...
function ScienceGradeCalculator() {
  ...
  const data = [
    { title: t("khmerLang"), img: p7, max: 75 },
    { title: t("math"), img: p4, max: 125 },
    { title: t("biology"), img: p6, max: 75 },
    { title: t("history"), img: p2, max: 50 },
    { title: t("chemistry"), img: p3, max: 75 },
    { title: t("physics"), img: p5, max: 75 },
    { title: t("foreignLang"), img: p1, max: 50 },
  ];

  const scores = scoresByTrack?.science || defaultScores;
        ...
        <div className="flex flex-col gap-8">
              ...
              <div className="flex flex-col-reverse justify-between gap-4 md:gap-6 md:flex-row md:items-center">
                <div className="flex items-center justify-between w-full md:w-auto">
                  ...
                  <button
                    ...
                    className={`flex md:hidden h-10 w-10 items-center justify-center rounded-xl transition-all active:scale-95 ${isDark ? "bg-white/5 text-blue-300" : "bg-accent/10 text-accent"
                      }`}
                ...
                <div className="flex justify-between items-center gap-3 rounded-2xl ring-1 bg-slate-200/50 p-1 md:p-1.5 dark:bg-white/5 sm:p-2">
                  ...
                  <button
                    onClick={() => document.getElementById('history-section').scrollIntoView({ behavior: 'smooth' })}
                    className={`hidden md:flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl transition-all hover:scale-105 active:scale-95 ${isDark ? "bg-white/5 text-blue-300" : "bg-accent/10 text-accent"
                      }`}
                    title={t("monthlyHistory")}
                  >
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_1.8fr]">
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={handleOpen}
                className={`group flex items-center justify-center gap-2 rounded-2xl py-3.5 text-base font-bold shadow-md transition-all active:scale-95 ${isDark
              ...
              <button
                onClick={() => { resetScores("science"); setValidationErrors({}); }}
                className={`group flex items-center justify-center gap-2 rounded-2xl py-3.5 text-base font-bold transition-all active:scale-95 ${isDark
            ...
            <div className={`px-8 pb-8 pt-4 rounded-t-[2.5rem] -mt-6 relative z-10 border-t ${isDark ? "bg-dark-primary border-white/5" : "bg-slate-50 border-white"}`}>
              <div className="flex flex-col items-center -mt-16 mb-6">
                ...
                <div className="mt-3 flex flex-col items-center">
                  ...
                  <span className={`mt-1 px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest ${gradeDetails.grade !== 'F'
              ...
              <div className="space-y-3">
                {data.map((item, idx) => {
                  // Assuming max scores based on user edits which were removed but I added fallback in logic
                  // data[idx].max is used for percentage calculation
...
export default ScienceGradeCalculator;
